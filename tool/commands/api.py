"""Internal API layer — the contract between CLI/TUI consumers and the app.

Every api_* function executes the business logic and returns a JSON-compatible
...
def emit_from_api(
    ...
    *,
    ...
    """CLI Consumer helper: emit an api envelope as a CLI JSON envelope.
...
def api_registry_list_available(wsl_name: str, user: str) -> dict:
...
def api_use_list_mounted(wsl_name: str, user: str) -> dict:
    ...
    *,
    ...
    try:
        ...
        for reg_id, _, _ in mounted:
            res = api_use_disable(reg_id, wsl_id)
            if status_of(res) == 0:
...
def api_install_database(*, force: bool = False) -> dict:
...
def api_alias_add(names: list[str]) -> dict:
    """Add aliases to shell profile files."""
    return _alias_mutate("add", names)


def api_alias_remove(names: list[str]) -> dict:
...
def api_start_prepare(wsl_name: str, user: str) -> dict:
